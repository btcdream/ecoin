...
//uint64_t nHPSTimerStart;

void BitcoinMiner(CWallet *pwallet, bool fProofOfStake)
{
  //  uint32_t *pdata
    uint32_t *scratchbuf = NULL;//scrypt_buffer_alloc();
        ...
        while(1)
        {
            ...
            if (nNonceFound != (unsigned int) -1)
            {
                if (result <= hashTarget)
                {
                    ...
                    if (!pblock->SignBlock(*pwalletMain,0))
                        break;
